...
import { Sale, SaleReturn, TreasuryTransaction, Purchase, PurchaseReturn, Supplier } from '../../types';
import { UsersIcon, PhoneIcon, MapPinIcon, EyeIcon, ArrowsUpDownIcon, CalculatorIcon } from '../icons';
...
const CustomerProfile: React.FC = () => {
  ...
  const { customers, sales, saleReturns, treasury, suppliers, purchases, purchaseReturns } = useContext(DataContext);
  ...
  const { statementData, party, linkedSupplier, openingBalance } = useMemo(() => {
    if (!id) return { statementData: [], party: null, linkedSupplier: null, openingBalance: 0 };
    const party = customers.find((c: any) => c.id === id);
    if (!party) return { statementData: [], party: null, linkedSupplier: null, openingBalance: 0 };

    const linkedSupplier = party.linkedSupplierId ? suppliers.find((s: Supplier) => s.id === party.linkedSupplierId) : null;
    ...
    if (isUnified && linkedSupplier) {
        allTx = [
            ...allTx,
            ...purchases.filter((t: Purchase) => t.supplier === linkedSupplier.name && !t.isArchived).map((t: Purchase) => ({
                date: t.date, id: t.id, description: `فاتورة مشتريات #${t.id} (مورد)`,
                debit: 0, credit: t.total, type: 'purchase', original: t
            })),
            ...purchaseReturns.filter((t: PurchaseReturn) => t.supplier === linkedSupplier.name && !t.isArchived).map((t: PurchaseReturn) => ({
                date: t.date, id: t.id, description: `مرتجع مشتريات #${t.id} (مورد)`,
                debit: t.total, credit: 0, type: 'purchaseReturn', original: t
            })),
            ...treasury.filter((t: TreasuryTransaction) => t.partyType === 'supplier' && t.partyId === linkedSupplier.id && !t.isArchived).map((t: TreasuryTransaction) => ({
                date: t.date, id: t.id, description: `${t.description} (مورد)`,
                debit: t.type === 'سند صرف' ? Math.abs(t.amount) : 0, 
                credit: t.type === 'سند قبض' ? Math.abs(t.amount) : 0, 
                type: 'treasury', original: t
            }))
    ...
    const openingBalance = (isUnified && linkedSupplier ? (party.balance - linkedSupplier.balance) : party.balance) - totalChange;
    ...
    return { statementData: statementWithBalance, party, linkedSupplier, openingBalance };
  }, [id, customers, sales, saleReturns, treasury, isUnified, suppliers, purchases, purchaseReturns]);


  if (!party) return <div className="p-8 text-center">لم يتم العثور على العميل.</div>;
  
  const getRowClass = (debit: number) => debit > 0 ? 'bg-red-50/50 dark:bg-red-900/10' : 'bg-green-50/50 dark:bg-green-900/10';
    ...
    <div className="space-y-4 md:space-y-6 px-0 md:px-0">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 md:gap-6">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-blue-100 dark:bg-blue-900/40 rounded-full shrink-0">
                <UsersIcon className="w-6 h-6 md:w-8 md:h-8 text-blue-500" />
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 md:gap-4 w-full lg:w-auto">
            <div className="text-center p-2 md:p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <p className="text-[10px] md:text-xs text-gray-500 mb-1">رصيده كعميل</p>
                <p className="font-bold font-mono text-sm md:text-base text-red-500">{party.balance.toLocaleString()}</p>
            </div>
            {linkedSupplier ? (
                <>
                    <div className="text-center p-2 md:p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <p className="text-[10px] md:text-xs text-gray-500 mb-1">رصيده كمورد</p>
                        <p className="font-bold font-mono text-sm md:text-base text-green-600">{linkedSupplier.balance.toLocaleString()}</p>
                    </div>
                    <div className="col-span-2 sm:col-span-1 text-center p-2 md:p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-100 dark:border-blue-800">
                        ...
                        <p className={`font-bold font-mono text-base md:text-xl ${(party.balance - linkedSupplier.balance) >= 0 ? 'text-red-500' : 'text-green-600'}`}>
                            {(party.balance - linkedSupplier.balance).toLocaleString()}
                        </p>
                    </div>
                </>
            ) : (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between p-3 md:p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-xl gap-3">
            <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                <span className="text-sm md:text-base font-semibold text-indigo-900 dark:text-indigo-100">
                  مرتبط بمورد: <span className="underline">{linkedSupplier.name}</span>
                </span>
            </div>
            <button 
            ...
            >
                {isUnified ? 'إخفاء حركات المورد' : 'عرض كشف الحساب الموحد'}
            </button>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden border dark:border-gray-700">
          ...
          <table className="w-full text-right">
            <thead className="bg-gray-50 dark:bg-gray-700/50">
              <tr>
                ...
                <th className="px-4 py-4 text-sm font-bold text-gray-600 dark:text-gray-300">مدين</th>
                <th className="px-4 py-4 text-sm font-bold text-gray-600 dark:text-gray-300">دائن</th>
            ...
            <tbody>
              ...
              {statementData.map((tx, index) => (
                <tr key={`${tx.id}-${index}`} className={`border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${getRowClass(tx.debit)}`}>
        ...
        <div className="md:hidden divide-y divide-gray-200 dark:divide-gray-700">
            ...
            {statementData.map((tx, index) => (
                <div key={`${tx.id}-${index}`} className={`p-4 ${getRowClass(tx.debit)}`}>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                        <div className="flex-1 bg-white/50 dark:bg-black/20 p-2 rounded text-center border border-black/5">
                            <p className="text-[10px] text-gray-500 uppercase font-bold">مدين (+)</p>
                            <p className="font-mono font-bold text-red-600">{tx.debit > 0 ? tx.debit.toLocaleString() : '-'}</p>
                        </div>
                        <div className="flex-1 bg-white/50 dark:bg-black/20 p-2 rounded text-center border border-black/5">
                            <p className="text-[10px] text-gray-500 uppercase font-bold">دائن (-)</p>
                            <p className="font-mono font-bold text-green-600">{tx.credit > 0 ? tx.credit.toLocaleString() : '-'}</p>
                        </div>
